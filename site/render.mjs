...
const brands = [
  ...
  { id: 'alago', name: 'ALAGO', title: 'Clarity for your working day.', description: 'Straightforward information, right on your phone.', label: 'Your day. A clearer next step.', hero: 'Your day.<br><em>A clearer view.</em>', body: 'Between work and the rest of your day, keep your loan information close. Explore ALAGO and see what to review before you apply.', scene: 'A woman in a blue blouse using her smartphone in a neighbourhood shop.' },
  { id: 'sulivo', name: 'SULIVO', title: 'Keep everyday plans in view.', description: 'A clearer look at borrowing, wherever the day takes you.', label: 'Everyday plans, thoughtfully managed.', hero: 'Your everyday.<br><em>In focus.</em>', body: 'From workdays to running your own business, everyday plans need a little room. Meet SULIVO and take a clear look at borrowing.', scene: 'A woman in a lavender shirt checking her phone beside stocked shop shelves.' },
];
const route = brand => siteUrl(`${brand.id}financing/`);
const arrow = '<span aria-hidden="true">↗</span>';
function header(active = 'home') {
  return `<a class="skip" href="#main">Skip to content</a><header class="site-header"><div class="wrap header-inner"><a class="wordmark" href="${siteUrl('#top')}" aria-label="Flashbit home">Flashbit<span class="wordmark-period">.</span></a><nav aria-label="Main navigation"><a href="${siteUrl('#top')}" ${active === 'home' ? 'aria-current="page"' : ''}>Home</a><a href="${siteUrl('#brands')}" ${active === 'brands' ? 'aria-current="page"' : ''}>Our Brands</a><a href="#contact">Contact us</a></nav></div></header>`;
}
function footer() {
...
function shell(title, description, body, brand = null) {
  const favicon = brand ? siteUrl(`assets/${brand.id}-icon.png`) : 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"%3E%3Crect width="64" height="64" rx="16" fill="%23183F42"/%3E%3Ctext x="15" y="48" font-family="Arial" font-size="48" font-weight="bold" fill="white"%3Ef%3C/text%3E%3C/svg%3E';
  return `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#183f42"><title>${title}</title><meta name="description" content="${description}"><link rel="icon" href='${favicon}'><link rel="stylesheet" href="${siteUrl('styles.css')}"></head><body id="top" class="${brand ? `brand-page ${brand.id}` : 'home-page'}">${header(brand ? 'brands' : 'home')}<main id="main">${body}</main>${footer()}</body></html>`;
}
function home() {
...
function brandPage(b) {
  ...
  <section class="brand-hero"><div class="wrap brand-hero-grid"><div class="brand-hero-copy"><div class="brand-identity"><img src="${siteUrl(`assets/${b.id}-icon.png`)}" width="360" height="360" alt=""><div><p class="brand-name">${b.name}</p><p class="brand-parent">A Flashbit brand</p></div></div><h1>${b.hero}</h1><p class="brand-intro">${b.body}</p><a class="primary-link" href="#features">Meet ${b.name}<span aria-hidden="true">↓</span></a><p class="hero-small-note">Simple. Clear. At your pace.</p></div><figure class="brand-hero-image">${portrait(b, '', 'eager')}<figcaption><span>${b.label}</span><span class="caption-line" aria-hidden="true"></span></figcaption></figure></div></section>
